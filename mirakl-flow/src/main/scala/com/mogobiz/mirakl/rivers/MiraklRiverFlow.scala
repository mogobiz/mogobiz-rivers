...
import rx.functions.{Func0, Func1}
...
object MiraklRiverFlow {
  ...
  def publish[In, Out](rivers: MiraklRivers[In, Out], config: RiverConfig, balanceSize: Int = 2, bulkSize: Int = 100, subscriber: Subscriber[Out]): Unit = {
    ...
    subscribe({
      Source() { implicit b =>
        ...
        val bulk = Flow[Seq[In]].mapAsyncUnordered[Out](items => rivers.bulk(config, items.toList, system.dispatcher))
  ...
  def synchronize[Entity, In, Out](river: MiraklRiver[In, Out], config: RiverConfig, balanceSize: Int = 2, bulkSize: Int = 100, subscriber: Subscriber[Out]): Unit = {
    val rivers = new MiraklRivers[In, Out] {
      override def loadRivers: List[MiraklRiver[In, Out]] = List[MiraklRiver[In, Out]](river)
      override def publisher(config: RiverConfig): Publisher[In] = river.publisher(config)

      import java.util

      override def bulk(config: RiverConfig, items: util.List[In], ec: ExecutionContext): Future[Out] = river.bulk(config, items, ec)
    }
    publish(rivers, config, balanceSize, bulkSize, subscriber)
  }
}

trait MiraklRivers[In, Out]{
  def loadRivers: List[MiraklRiver[In, Out]]
...
abstract class AbstractMiraklRiver[Entity, In, Out] extends MiraklRiver[In, Out]{
